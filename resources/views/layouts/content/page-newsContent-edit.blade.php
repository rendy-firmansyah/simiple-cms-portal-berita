...
<form method="POST" action="{{route('news-content.update', $content->id)}}" enctype="multipart/form-data">
    @method('PUT')
    @csrf
    <div class="row">
        <div class="col-md-6 mb-3">
            <label for="title" class="form-label">News Headline</label>
            <input type="text" class="form-control" id="title" name="headline" placeholder="Masukkan judul berita" required value="{{$content->headline}}">
        </div>
    
        <div class="col-md-6 mb-3">
            <label for="date" class="form-label">Date Created</label>
            <input type="date" class="form-control" id="date" name="date_created" required value="{{$content->date_created}}">
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-6 mb-3">
            ...
            <input type="file" class="form-control" id="image" name="upload_image" accept="image/*">
            @if ($content->upload_image)
            <input type="hidden" name="existing_image" value="{{ $content->upload_image }}">
            <p>Current Image: {{ $content->upload_image }}</p>
            @endif
        </div>
        <div class="col-md-6 mb-3">
            ...
            <select class="form-control col-md-15" name="categories_id" id="">
                ...
                @foreach ($categories as $item)
                    <option value="{{$item->id}}" {{$item->id == $content->categories_id ? 'selected' : ''}}>{{$item->name}}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="mb-3">
        ...
        <textarea class="form-control" id="content" name="news_content" rows="5" placeholder="Masukkan isi berita" required >{{$content->news_content}}</textarea>
    </div>
